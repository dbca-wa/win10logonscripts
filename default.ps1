...
$mapdrivescmd = "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\StartUp\driveletters.cmd";
Remove-Item $mapdrivescmd -Force;
foreach ($letter in $($($env:OneDriveLetterEmulation | Select-String -Pattern '^[A-Z]+$') -join "").toCharArray()) {
    write-output "subst ${letter}: c:\programdata\onedrive" >> $mapdrivescmd
}
